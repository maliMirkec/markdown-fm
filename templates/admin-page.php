...
<div class="wrap">
  <div class="markdown-fm-admin-container">
    <div class="markdown-fm-header">
      <div class="markdown-fm-header-content">
        ...
        <div class="markdown-fm-header-text">
          ...
          <p class="markdown-fm-tagline"><?php esc_html_e('YAML-powered content schemas for WordPress themes', 'markdown-fm'); ?></p>
        </div>
      </div>
    </div>

    <div class="markdown-fm-intro">
    <p><?php esc_html_e('Markdown FM allows you to define YAML frontmatter schemas for your theme templates. Enable YAML for templates, define schemas, and manage structured content directly in the WordPress editor.', 'markdown-fm'); ?></p>
    <p><strong><?php esc_html_e('Inspired by', 'markdown-fm'); ?> <a href="https://pagescms.org/docs/" target="_blank">PagesCMS</a></strong></p>
    <p>
      <a href="<?php echo esc_url(add_query_arg('refresh_mdfm', '1')); ?>" class="button">
        <span class="dashicons dashicons-update"></span>
        <?php esc_html_e('Refresh Template List', 'markdown-fm'); ?>
      </a>
    </p>
    <p class="description">
      <?php esc_html_e('Scan theme files for new templates and partials with @mdfm markers', 'markdown-fm'); ?>
    </p>
    <p>
      <button type="button" class="button markdown-fm-export-settings">
        <span class="dashicons dashicons-download"></span>
        <?php esc_html_e('Export Settings', 'markdown-fm'); ?>
      </button>
      <button type="button" class="button markdown-fm-import-settings-trigger" style="margin-left: 10px;">
        <span class="dashicons dashicons-upload"></span>
        <?php esc_html_e('Import Settings', 'markdown-fm'); ?>
      </button>
      <input type="file" id="markdown-fm-import-file" accept=".json" style="display: none;" />
    </p>
    <p class="description">
      <?php esc_html_e('Backup or restore all schemas and settings', 'markdown-fm'); ?>
    </p>
    </div>

    <h2><?php esc_html_e('Page Templates', 'markdown-fm'); ?></h2>
    <p><?php esc_html_e('Configure YAML schemas for page templates. Data for these templates is stored per post/page.', 'markdown-fm'); ?></p>

    <?php if (empty($templates)) : ?>
    <p><?php esc_html_e('No templates found in the current theme.', 'markdown-fm'); ?></p>
    <?php else : ?>
    <div class="wp-table-wrap">
      <table class="wp-list-table widefat striped">
        <thead>
            <tr>
                <th><?php esc_html_e('Template Name', 'markdown-fm'); ?></th>
                <th><?php esc_html_e('File', 'markdown-fm'); ?></th>
                <th><?php esc_html_e('Enable YAML', 'markdown-fm'); ?></th>
                <th><?php esc_html_e('Schema', 'markdown-fm'); ?></th>
            </tr>
        </thead>
        <tbody>
                    ...
                    <td>
                        <?php if ($is_enabled) : ?>
                            <a href="<?php echo esc_url(admin_url('admin.php?page=markdown-fm-edit-schema&template=' . urlencode($template['file']))); ?>"
                              class="button">
                                <?php echo $has_schema ? esc_html__('Edit Schema', 'markdown-fm') : esc_html__('Add Schema', 'markdown-fm'); ?>
                            </a>
                            <?php if ($has_schema) : ?>
                                <span class="dashicons dashicons-yes-alt" style="color: #46b450;"></span>
                            <?php endif; ?>
                        <?php else : ?>
                            <span class="description"><?php esc_html_e('Enable YAML first', 'markdown-fm'); ?></span>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
    ...
    <h2 style="margin-top: 40px;"><?php esc_html_e('Template Partials', 'markdown-fm'); ?></h2>
    <p><?php esc_html_e('Configure YAML schemas for template partials (header, footer, sidebar, etc.). Data for partials is stored globally and can be managed below.', 'markdown-fm'); ?></p>

    <?php if (empty($partials)) : ?>
    <p><?php esc_html_e('No partials found in the current theme.', 'markdown-fm'); ?></p>
    <?php else : ?>
    <div class="wp-table-wrap">
      <table class="wp-list-table widefat striped">
        <thead>
            <tr>
                <th><?php esc_html_e('Partial Name', 'markdown-fm'); ?></th>
                <th><?php esc_html_e('File', 'markdown-fm'); ?></th>
                <th><?php esc_html_e('Enable YAML', 'markdown-fm'); ?></th>
                <th><?php esc_html_e('Schema', 'markdown-fm'); ?></th>
                <th><?php esc_html_e('Data', 'markdown-fm'); ?></th>
            </tr>
        </thead>
        <tbody>
                    ...
                    <td>
                        <?php if ($is_enabled && $has_schema) : ?>
                            <a href="<?php echo esc_url(admin_url('admin.php?page=markdown-fm-edit-partial&template=' . urlencode($partial['file']))); ?>"
                              class="button">
                                <?php esc_html_e('Manage Data', 'markdown-fm'); ?>
                            </a>
                        <?php else : ?>
                            <span class="description"><?php esc_html_e('Add schema first', 'markdown-fm'); ?></span>
                        <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
    ...
    <h2><?php esc_html_e('Schema Example', 'markdown-fm'); ?></h2>
    <p><?php esc_html_e('Here\'s an example schema in YAML format:', 'markdown-fm'); ?></p>
    <pre>fields:
  - name: title
    label: Page Title
    type: string
  ...
  - name: sections
    ...
    <h3><?php esc_html_e('Supported Field Types', 'markdown-fm'); ?></h3>
    <ul>
